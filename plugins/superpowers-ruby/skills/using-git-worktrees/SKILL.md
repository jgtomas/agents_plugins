...
if [ -f Gemfile ] && [ -f config/database.yml ] && grep -q "adapter: sqlite3" config/database.yml; then
  ...
  # Resolve scripts/create_sqlite_worktree.rb from the using-sqlite-worktrees
  # skill directory, then invoke that resolved path from MAIN_WORKTREE.
  SQLITE_HELPER="<using-sqlite-worktrees skill directory>/scripts/create_sqlite_worktree.rb"
  (cd "$MAIN_WORKTREE" && ruby "$SQLITE_HELPER" "$WORKTREE_PATH")
  SQLITE_EXIT=$?

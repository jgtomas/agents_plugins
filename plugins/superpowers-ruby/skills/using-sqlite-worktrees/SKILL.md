...
From the main working directory (not the worktree), resolve
`scripts/create_sqlite_worktree.rb` relative to this skill's directory and run
the helper script:

```bash
ruby "<using-sqlite-worktrees skill directory>/scripts/create_sqlite_worktree.rb" <worktree-path>
```
...
If the main branch's dev data has moved forward and you want to refresh a worktree's `storage/`, resolve the same helper from the `using-sqlite-worktrees` skill directory:

```bash
# Same script, same command. Existing worktree files get backed up as .bak before overwrite.
ruby "<using-sqlite-worktrees skill directory>/scripts/create_sqlite_worktree.rb" <path-to-existing-worktree>
```
...
[Runs the resolved using-sqlite-worktrees skill helper at
`scripts/create_sqlite_worktree.rb` against `.worktrees/feature-x`]

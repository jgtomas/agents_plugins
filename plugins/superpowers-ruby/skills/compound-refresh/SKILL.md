...
Inspect the user's request for `mode:autonomous`. If present, remove that marker
from the request (using any remaining wording as a scope hint) and run in
**autonomous mode**.

| Mode | When | Behavior |
|------|------|----------|
| **Interactive** (default) | User is present and can answer questions | Ask for decisions on ambiguous cases, confirm actions |
| **Autonomous** | `mode:autonomous` in the user's request | No user interaction. Apply all unambiguous actions (Keep, Update, auto-Archive, Replace with sufficient evidence). Mark ambiguous cases as stale. Generate a summary report at the end. |
...
If the user supplied a scope hint in the request, use it to narrow scope before
proceeding. Try these matching strategies in order, stopping at the first that
produces results:
